...
#    self.EncodedReadings = [[0]*D for _ in range(N)]
  ...
  def getCommitedReadings(self,u):
    for i in range(0,1):
      self.ComittedReadings= point_add(scalar_mult(usersTupples[u][i][0],curve.g),scalar_mult(5,curve.g))
    return self.ComittedReadings

  # InnerProducts functionl encryption (meater readings)
  def getIpfeEncryptedReading(self,u):
      for i in range(1): # 10 meter readings for each of the 10 periods
...
class Supplier:
  ...
  def setEncryptedData(self,u):
        ...
        end_time = time.time()
        print("MAsking time is:", end_time  -  start_time)
        self.maskedPTypes, self.maskedCTypes = self.MO.getMaskedPTypes(u), self.MO.getMaskedCTypes(u)
...
def devWeight():
    for i in range(1): # number of periods
